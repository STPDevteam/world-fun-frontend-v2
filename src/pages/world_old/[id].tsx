...
//   const { id } = use(params);
  ...
  const worldData = useWorldData(address, 'sharktank', refresh)
  const {
    control,
    setValue,
    getValues,
  ...
  const renderFundraiseSection = () => (
    <Box width={{base:'100%',md:'446px'}}>
      <Flex justifyContent="flex-end" display={{base:'none',md:'flex'}} height={{base:'auto',md:'309px'}} position="relative" mt={{ base: 6, md: 0 }} >
        <Image
        ...
        <Box
          ...
          onClick={() => {router.push('/live/SHARKTANK')}}
        >
          Enter World
        </Box>
      </Flex>
      ...
      >
        ...
        <a
          href="https://aerodrome.finance/swap?from=0x833589fcd6edb6e08f4c7c32d4f71b54bda02913&to=0x1b4617734c43f6159f3a70b7e06d883647512778&chain0=8453&chain1=8453"
          target="_blank"
        ...
        >
          <Box
            ...
            _hover={{ border: '1px solid #80C838', color: '#80C838' }}
            cursor="pointer"
            minW="220px"
            maxW="320px"
          >
            Get $AWE Token
            <Image src="/link.svg" alt="token" />
          </Box>
        </a>
      </Box>
      <Box border="1px solid rgba(224, 224, 224, 0.2)" fontWeight={'light'} fontFamily={'DMMono'} p={{ base: 3, md: 4 }} borderRadius={'10px'} mt='30px'>
        ...
        {remainingTime && <Flex mt={4} justify="space-between" gap={{ base: 2, md: 0 }} mb={'32px'}>
          ...
          <Status.Root size={{ base: "sm", md: "lg" }} colorPalette="green">
            {worldData?.whitelisted ? <Icon size="sm" color="green"><FaRegCheckCircle /></Icon> : <Icon size="sm" color="white"><PiWarningCircle/></Icon> }
            {worldData?.whitelisted ? <Text fontSize={{ base: "xs", md: "sm" }}>Approved</Text>: <Text fontSize={{ base: "xs", md: "sm" }}>Public</Text>}
          </Status.Root>
        </Flex>}
        <form onSubmit={handleContribute}>
          {remainingTime && <Box my={4} border="1px solid rgba(224, 224, 224, 0.2)" p={{ base: 3, md: 4 }} mb={'32px'} borderRadius={'10px'}>
            ...
            <Flex justify="space-between">
              <Group >
                ...
                <Button borderRadius={'10px!important'} variant="ghost" fontSize={'18px'} color="#666" onClick={() => { setValue('amount', aweBalance) }}>
                  Max
                </Button>
              </Group>
              <HStack>
          ...
          <Flex justify="space-between" gap={{ base: 1, md: 0 }} mt={1} mb={!remainingTime && !isTradable ? 0 : '32px'}>
            ...
            <Text fontSize={{ base: "xs", md: "sm" }}>{worldData.estimated_allocation} SHARK</Text>
          </Flex>
          {remainingTime && 
            <>{isConnected ? (
        ...
        <Grid templateColumns={{ base: "repeat(1, 1fr)", md: 'repeat(3, 1fr)' }} gap={{ base: 4, md: 6 }}>
          {
            ...
            <GridItem colSpan={{ base: 1, md: 2 }}>
              <Box height={{base:'auto',md:'309px'}}  mb={4}>
                <Heading as="h2" fontSize={{ base: "xl", md: "3xl" }} fontFamily={'BDO Grotesk'} lineHeight={{ base: "1.2", md: "1.3" }}>
                  AI Shark Tank
                </Heading>
                <Text fontSize={{ base: "xs", md: "sm" }} color="#80C838" fontFamily={'DMMono'} mt={{ base: 1, md: 2 }} mb={{ base: 3, md: 4 }}>
                {/* Launch Date: {moment(Number(startTime?startTime:'1751374800')*1000).format('YYYY-MM-DD HH:mm:ss')} */}
